...
exec_params = {
    ...
    'output_path':'/test/output',    #Path to the output file where the extracted trip descriptions will be stored
...
# Context settings
context_settings = {
    'vessel_info':'/test/settings/vessel_info.csv',  # CSV: The type(passenger, cargo, tanker, etc.) of each vessel
    'ports':'/test/context/ports.csv', # CSV: List of ports in the area and their names
    'placemarks':'/test/context/osm_places.shp', # SHP: The shapefile containing cislands and other (polygon) placemarks as extracted from OSM
    'protected_areas':'/test/context/osm_protected_areas.shp', # SHP: The shapefile containing protected areas as extracted from OSM
    'capes':'/test/context/osm_capes.shp', # SHP: The shapefile containing capes, peninsuale, straits, etc as extracted from OSM
    'ferry_routes':'/test/context/osm_ferry_routes.shp', # SHP: The shapefile containing ferry routes as extracted from OSM
    'separation_zones':'/test/context/osm_separation.shp', # SHP: The shapefile containing separation zones as extracted from OSM
    'meteo_netcdf':'/test/context/meteo',   # Path to the NetCDF files containing meterorological (wind) data
    'bytho_netcdf':'/test/context/bythometry',   # Path to the NetCDF files containing meterorological (wind) data
}
...
ais_annotated_file = '/test/input/ais_annotated_locations.csv'
